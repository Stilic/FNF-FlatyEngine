...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var rankText:FlxText = new FlxText(0, 10, 0, 'RANK: GREAT');
		...
		for (i in 0...weekData.length)
		{
			...
			weekThing.antialiasing = PreferencesMenu.getPref('antialiasing');
			...
			if (!weekUnlocked[i])
			{
				...
				lock.ID = i;
				lock.antialiasing = PreferencesMenu.getPref('antialiasing');
				grpLocks.add(lock);
			}
		}
		...
		for (char in 0...3)
		{
			...
			weekCharacterThing.antialiasing = PreferencesMenu.getPref('antialiasing');
	...
	function updateText()
	{
		...
		var cock:String = "Tracks\n";
		for (i in weekData[curWeek])
		{
			cock += "\n" + i;
		}
		cock = cock.toUpperCase();
		cock += "\n";

		txtTracklist.text = cock;
